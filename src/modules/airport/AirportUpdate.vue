        ...
        <v-form @submit.prevent="save">
          <v-card class="pa-4">
            ...
            <div class="text-xs-center pa-4">
              <h2 class="font-weight-light"><v-icon medium>flight</v-icon> Update Airport</h2>
            </div>
...
<script>
import { AirportAPI, WoredaAPI, AirportTypeAPI } from "@/api";
...
export default {
  ...
  async created() {
    const { airportId } = this.$route.params;
    AirportAPI.get(airportId).then(res => {
      this.item = res;
    });
  },
  methods: {
